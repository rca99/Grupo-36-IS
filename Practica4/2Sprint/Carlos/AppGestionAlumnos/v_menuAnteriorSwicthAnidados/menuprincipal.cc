...
void OpcionesOrden();
void elegirOpcionesOrden();
void limpiarPantalla();
void menuMostrarAlumnos();
void menuprincipal();
					...
					switch(opcMostrar) {
						case 1: {
							...
							*/
							...
							getchar();// se pone antes de usar cualquier getline
							cout<<COLOR_DARKGREY<<"Indique el DNI del alumno que desea mostrar: "<<COLOR_BLUE;getline(cin,DNI);
							z.setDNI(DNI);	
							cout<<RESET<<endl;
							...
							if (miBD.buscarAlumnoDNI(listaBusqueda, z)){
								...
								if (listaBusqueda.size()>1){
									cout<<COLOR_RED<<"**ATENCIÓN: HAY ALUMNOS DUPLICADOS**"<<"Se han encontrado "<<listaBusqueda.size()<<" alumnos con DNI: "<<z.getDNI()<<RESET<<endl;
								...
								elegirOpcionesOrden();
								miBD.mostrarAlumno(listaBusqueda, 1, "2");
...
void elegirOpcionesOrden(){
	...
	}while ((opcOrden!=1 ||opcOrden!=2||opcOrden!=3||opcOrden!=4||opcOrden!=5)&& (orden!="ASC"||orden!="DES"));
